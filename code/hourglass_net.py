...
def hnet_3stacks_no_incep(netIN,training):
	print('-'*30)
	...
	def hourglass_stack_no_incep(stack_in):
		...
		stack_out = conv_layer(c16,layer_name('conv'),1,3,bn=False,training=training,relu=False)
		return stack_out

	out0 = hourglass_stack_no_incep(netIN)
	out1 = hourglass_stack_no_incep(tf.concat([out0, netIN], 3))
	out2 = hourglass_stack_no_incep(tf.concat([out1,out0,netIN],3))
